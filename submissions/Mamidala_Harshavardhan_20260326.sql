 1.Write a query to display dept details which contains employees.
 
SELECT * FROM DEPT d
WHERE EXISTS (SELECT 1 FROM EMP e WHERE e.DEPTNO = d.DEPTNO);

2.Write a query to display Employees whose salary is greater than or equal to the total salary of ANY department

SELECT * FROM EMP 
WHERE SAL >= ANY (SELECT SUM(SAL) FROM EMP GROUP BY DEPTNO);

3.Write a query to display the Department information containing more than 3 employees.
...
4.Write a query to display the Max salary for CLERKS in  20 department.
...
7.Write a query to display employees info. who are working as IT in INDIA and MECH from AUS.
