...
s = input("Enter a string: ")  # Example: babble
first_char = s[0]
out = first_char
...
a = input('Enter an expression : ').split('+')
sum = 0
for i in a :
    sum += int(i)
print(sum)
...
#e = [15 , 36 , 48]
#print(':' . join(e)) #Error elements must be in strigs 
...
print('' . isdigit()) #   False
print(' ' . isdigit()) # False
#print(9247 . isdigit()) #Error must be string
...
s = input("Enter a string: ")  # Example: Z9K3PA7D51
...
alpha_sorted = ''.join(sorted(alpha))
digit_sorted = ''.join(sorted(digit))

# Concatenate
result = alpha_sorted + digit_sorted
