...
for x in zip(a , b):
	print(x)  # ('Telangana', 'Hyderabad') <nextline>('Andhra Pradesh', 'Amaravathi')<nextline>('Karnataka ', 'Bangalore')<nextline># ('Tamilnadu', 'Chennai')
	time.sleep(1)
print('Elements of each tuple in zip object') # Elements of each tuple in zip object
for x , y in zip(a , b):
	print(x , y)  # Telangana Hyderabad <nextline>Andhra Pradesh Amaravathi <nextline>Karnataka Bangalore<nextline> Tamilnadu Chennai
	time.sleep(1)
...
while True:
	try:
		print(next(z)) # ('Empno', 25)<nextline>('Emp Name', 'Rama Rao')<nextline>('Salary', 10000.0)
		time.sleep(1)
...
print(r1) # <reversed object at 0x1B8765>
...
for x in reversed(a):
	print(x) # D<nextline>Y<nextline>H
	time.sleep(1)
...
print(b) # <reversed object at 0x1A76543>
...
'''
'''
10) # How to print list_reverseiterator object in different ways (Home work)
import time
...
for x in reversed(lst):
	print(x) # True<nextline>10.8<nextline>25<nextline>
	time.sleep(1)
...
'''
'''
11) # Can set be reversed ? (Home work)
a = {10, 20, 15 , 18}
# r = reversed(a) # Error because set is not reversible
'''
'''
12) # Can dictionary be reversed ? (Home work)
import time
def disp(r):
	while True:
		try:
...
r3 = reversed(a.items())
disp(r3) # (18, 'Amar')<nextline>(15, 'Kiran')<nextline>(20, 'Sita')<nextline>(10, 'Rama')
...
print(list_itr1) # <list_iterator object at 0x1A234567>
...
print(a) # 25
for x in a:
	# print(x) # Error because int is not iterable
# print(iter(a)) # Error because int is not iterable
# print(next(a)) # Error because int is not iterable
'''