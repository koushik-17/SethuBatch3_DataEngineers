...
x . m3()  # error as there is no m3 method
x = child()
...
class  shape(ABC):
	def   get(self):
		 self.a = a # How  to  read  value  of  'a'
	@abstractmethod
	def   area(self):
		pass
	@abstractmethod
...
class   circle(shape):
	...
	def   area(self):
		return  3.14*self.a**2  # area  of  circle
	def   peri(self):
		return  2 * 3.14 * self.a # circumference  of circle
	def  test(self):
		if  side  is  -ve
...
class   rectangle(shape):
	def  get(self):
		...
		super().get() 
                self.b = b   # How  to  read  length  and  breadt		
	def   area(self):
...
def   operation(s):
	s.a = float(input("Enter side a : ")) 
        s.b = float(input("Enter side b : ")) 
        s.c = float(input("Enter side c : "))     # How  to  read  inputs  to  object  's'
	stest() # How  to  test  inputs  are  valid  (or)  not
	print('Area  :  ' ,  s.area())
