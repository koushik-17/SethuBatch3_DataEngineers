...
while True:
    try:
        identifier = next(it)   # get next input using next()
...
if  m:
        ...
        print('Different  strings')  #Different strings
----------------------------------------------------------------------------------
...
'''
import re
identifier = input('Enter  any  Phone number : ') 
m  = re . fullmatch('(0|[+]91)?[6789][0-9]{9}' ,  identifier)   #  Returns  Match  object  when  input  matches  with  reg  exp  and  None  otherwise
if   m:
	print('Valid')
else:
	print('Invalid')
...
import re
identifier = input('Enter  any  Vehicle Registration number : ') 
m  = re . fullmatch('(TS|Ts|tS|ts)(0[1-9]|1[0-9]|2[0-9])[A-Za-z]{2}[0-9]{4}' ,  identifier)   #  Returns  Match  object  when  input  matches  with  reg  exp  and  None  otherwise
if   m:
	print('Valid')
else:
	print('Invalid')
...
import re
identifier = input('Enter  any  Vehicle Registration number : ') 
m  = re . fullmatch('(0?[1-9]|[12][0-9]|3[01])/(0?[1-9]|1[0-2])/[0-9]{4}' ,  identifier)   #  Returns  Match  object  when  input  matches  with  reg  exp  and  None  otherwise
if   m:
	print('Valid')
else:
	print('Invalid')
