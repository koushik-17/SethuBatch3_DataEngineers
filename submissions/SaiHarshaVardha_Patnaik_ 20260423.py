...
while   True:
	...
	time . sleep(1)
'''
Infinite loop- while conditionis always True and cycle prints the loop continously
'''
...
.
.
Infinite times - while condition is always True
'''
...
digit = '0123456789'
for i in range(10):
    password = random.choice(letter) + random.choice(digit) + random.choice(letter) + random.choice(digit) + random.choice(letter) + random.choice(digit)
    print(password)
...
if __name__ == "__main__":
    directory_name = input("Enter the directory name or path: ")
