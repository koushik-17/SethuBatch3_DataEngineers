#  Find  outputs  (Home  work)
def  f1():
	print('f1  function')
def   f2(fun):
	print('f2  function')
	fun()
	print('Back  to  f2  function')
# end of the function
print('Begin')
f2(f1)
print('End')
'''
Begin
f2 function
f1 function
Back to f2 function
End'''


#  Find  outputs  (Home  work)
def  f1():
	print('f1  function')
def   f2 (fun):
	print('f2  function')
	fun()
	print('Back  to  f2  function')
# end of the function
print('Begin')
f2(f1())#error- the fun() returns f1(()) returns the value of none and f2(none) also returns error because there is no function with name none
print('End')

'''
Begin
f1 function
f2 function
'''


# Find  outputs (Home  work)
def   outer():
	print('Outer  Function')
	def  inner():
		print('Inner function')
	return   inner
# End  of  the  function
fun = outer()
print('Hello')
fun()
print('Bye')
#inner()#error-inner() is defined inside the outer function and hence it is hidden outside

'''
Outer Function
Hello
Inner Function
Bye'''




# Find  outputs (Home  work)
def  outer(x):#x=10 #x=20
	print('Outer  Function')
	def  inner1():
		print('1st  inner  function')
	# End  of  inner1  function
	def  inner2():
		print("2nd  inner  function")
	# End  of  inner2  function
	if   x == 10:
		return  inner1
	else:
		return  inner2
# End  of  the  outer  function
f1 = outer(10)
f2 = outer(20)
f1()
f2()
'''
Outer Function
Outer Function
1st  inner  function
2nd  inner  function
'''


# Find  outputs  (Home  work)
def   outer(msg):
	def  inner():
		print(msg)
	return  inner
# End  of  the  function
hi_fun = outer('Hi')
hello_fun = outer('Hello')
hi_fun()
hello_fun()

'''
Hi
Hello
'''


#  How  to  call  f1()  function  when  @decor  tag  is  missing  ?
def   decor(fun):#fun=f1
	def   inner():
		x = fun() #x== result of f1=10
		return   x + 2 #12
	return  inner
def  f1():
        return  10
# End  of  the  function
f1 = decor(f1)
print(f1())

'''
12
'''



# Find  outputs(Home  work)
def   decor(fun):#fun=wish #fun=wish
	print(fun . __name__)#wish #wish
	def    inner(name):#name=Python #name=Java
		if   name  == 'Python':
			print('Hello' , name)#Hello Python
		else:
			fun(name)#wish(Java)
	return  inner
@decor #wish=decorators(wish)
def    wish(name):#wish(Java)
        print('Hi' , name)# Hi Java
# End  of  the  function
wish('Python')#wish(inner)
wish('Java')#wish(inner)
'''
wish
Hello Python
Hi Java
'''



# Find  outputs(Home  work)
def   decor(fun):#fun=div
	def  inner(x , y):#x=10 y=3
		try:
			return  fun(x , y) #div(10,3)
		except:
			return   'Division   by  0  is  not  permitted'
	return  inner
@decor#div=decor(div)
def  div(a , b):#a=10 b=3
        return  a / b #10/3=3.33
# End  of  the  function
print(div(10 , 3))#3.33
print(div(10 , 0))#'Division   by  0  is  not  permitted'
print(inner(10 , 3))#error-because inner is hidden outside the function and cannot be called directly




# Modify  following  div  function  such  that  both  div(9 , 2)  and  div(2 , 9)  should  return  4.5  only
def  decor(fun):
	def  inner(x , y):
		if(x>y):
			return  fun(x , y)#9/2=4.5
		else:
			return   1/fun(x,y)#1/2/9=4.5
	return  inner
	
	#How  to  decorate  the  function  such  that  4.5  is  returned
@decor
def  div(a , b):
    return   a /b
print(div(9 , 2))  #  4.5
print(div(2 , 9))  # 4.5


#  Find  outputs (Home  work)
def   decor(fun):#fun=f1
	def   inner():
		print(F'Decorating  {fun . __name__}  function')
		fun()#f1
		print('Decoration  is  finished')
	return  inner
@decor#f1=decor(f1) 
def   f1():
	print('Hello')
# End  of  the  function
f1()
print('Bye')
'''
Decorating f1 function
Hello
Decoration  is  finished
Bye

'''



# Most  tricky   program
# Same  decorator  to  multiple  functions  with  different  signatures
def   decor(fun):
	print(fun . __name__)
	def   inner(*x):  #  *  is  var-arg  parameter
		print(x)
		fun(*x)  #  Unpacks  object  'x'
		print('End  of  decoration')
	return  inner
@decor
def   f1(x):
	print('f1  function  :  ' , x)
@decor
def   f2(x , y):
	print('f2  function  :  ' , x , y )
@decor
def  f3(x , y , z):
	print('f3 function : ' , x , y , z)
@decor
def   f4():
	print('f4 function')
# end of function
f1(10)
f2(25 , 10.8)
f3('Hyd' ,  True  , 3 + 4j)
f4()



# Find  outputs  (Home  work)
def  square(fun):#fun=num
	def  inner1():
		x = fun() #num()#x=10
		return  x * x #10*10=100
	return  inner1
def   double(fun):#fun=square(num)
	def  inner2():
		y = fun()#y=square(num)#y=100
		return  2 * y# 2*100=200
	return   inner2
@double #num=double(square(num))#num=double(100)
@square #num=square(num)
def  num():
	return  10
#end of the function
print(num())
'''
200
'''