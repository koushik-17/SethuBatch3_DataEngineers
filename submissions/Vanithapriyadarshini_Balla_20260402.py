#  Find  outputs  (Home  work)
def  f1():
	print('f1  function')
def   f2(fun):
	print('f2  function')
	fun()
	print('Back  to  f2  function')
# end of the function
print('Begin')# Begin
f2(f1)# f2 function <nl> f1 function <nl>  Back to f2 function
print('End') # End

# Find  outputs (Home  work)
def   outer():
	print('Outer  Function')
	def  inner():
		print('Inner function')
	return   inner
# End  of  the  function
fun = outer() # Outer Function <nl> fun = inner
print('Hello') # Hello
fun() # Inner Function 
print('Bye') # Bye
#inner() # Error

# Find  outputs (Home  work)
def  outer(x):
	print('Outer  Function')
	def  inner1():
		print('1st  inner  function')
	# End  of  inner1  function
	def  inner2():
		print("2nd  inner  function")
	# End  of  inner2  function
	if   x == 10:
		return  inner1
	else:
		return  inner2
# End  of  the  outer  function
f1 = outer(10) # Outer function , f1=inner1
f2 = outer(20) # Outer funcion , f2=inner2
f1() # 1st inner function
f2() # 2nd inner function

# Find  outputs  (Home  work)
def   outer(msg):
	def  inner():
		print(msg)
	return  inner
# End  of  the  function
hi_fun = outer('Hi')# hi_fun=inner, msg =Hi
hello_fun = outer('Hello')# hello_fun=inner, msg=Hello
hi_fun()# Hi
hello_fun()# Hello

#  How  to  call  f1()  function  when  @decor  tag  is  missing  ?
def   decor(fun):
	def   inner():
		x = fun()
		return   x + 2
	return  inner
def  f1():
        return  10
# End  of  the  function
f1 = decor(f1)# inner,fun=f1
print(f1()) # print(inner())=print(12)=12

# Find  outputs(Home  work)
def   decor(fun): # fun=wish
	print(fun . __name__) # wish
	def    inner(name):
		if   name  == 'Python':
			print('Hello' , name)
		else:
			fun(name)# = wish('Java')
	return  inner
@decor # wish=decor(wish),----> wish and  wish=inner
def    wish(name):
        print('Hi' , name)
# End  of  the  function
wish('Python') # inner('Python')--->  Hello Python
wish('Java') #  Hi Java

# Find  outputs(Home  work)
def   decor(fun): # fun is div
	def  inner(x , y):
		try:
			return  fun(x , y) # div(10,3)
		except:
			return   'Division   by  0  is  not  permitted'
	return  inner
@decor # div=decor(div), div=inner
def  div(a , b):
        return  a / b # 10/3=3.333
# End  of  the  function
print(div(10 , 3))# inner(10,3)---> 3.333
print(div(10 , 0))# inner(10,0)----> 'Division   by  0  is  not  permitted'
#print(inner(10 , 3)), bcz we cannot call inner() func directly

# Modify  following  div  function  such  that  both  div(9 , 2)  and  div(2 , 9)  should  return  4.5  only
def  decor(fun): # fun is div
	#How  to  decorate  the  function  such  that  4.5  is  returned
    def inner(a,b):
        if a>b:
            return fun(a,b)
        else:
            return fun(b,a)
    return inner
@decor # div=decor(div)
def  div(a , b):
    return   a /b
print(div(9 , 2))  #  4.5
print(div(2 , 9))  # 4.5

#  Find  outputs (Home  work)
def   decor(fun): # fun is f1
	def   inner():
		print(F'Decorating  {fun . __name__}  function')
		fun()
		print('Decoration  is  finished')
	return  inner
@decor # f1=decor(f1)--> f1=inner
def   f1():
	print('Hello')
# End  of  the  function
f1() # inner()---> Decorating f1 function <nl> Hello <nl> Decoration  is  finished
print('Bye') # Bye

# Most  tricky   program
# Same  decorator  to  multiple  functions  with  different  signatures
def   decor(fun): # fun is f1
	print(fun . __name__)
	def   inner(*x):  #  *  is  var-arg  parameter
		print(x)
		fun(*x)  #  Unpacks  object  'x', f1(*x)Error
		print('End  of  decoration')
	return  inner
@decor # f1=decor(f1)---> f1  , f1=inner
def   f1(x):
	print('f1  function  :  ' , x)
@decor # f2=decor(f2)---> f2    , f2=inner
def   f2(x , y):
	print('f2  function  :  ' , x , y )
@decor # f3=decor(f3)---> f3   , f3=inner
def  f3(x , y , z):
	print('f3 function : ' , x , y , z)
@decor # f4=decor(f4)---> f4    , f4=inner
def   f4():
	print('f4 function')
# end of function
f1(10)# inner(10)---> (10,)<nl> f1  function  :  (10,)<nl> End  of  decoration
f2(25 , 10.8) # inner(25,10.8)---> (25, 10.8)<nl> f2  function  : (25,10.8)<nl> End  of  decoration
f3('Hyd' ,  True  , 3 + 4j) # inner('Hyd' ,  True  , 3 + 4j)-->('Hyd' ,  True  , 3 + 4j)<nl> f2  function  : ('Hyd' ,  True  , 3 + 4j)<nl> End of decorator
f4()# inner()----> ()<nl> f4 function : ()<nl> End of decorator 

# Find  outputs  (Home  work)
def  square(fun): #fun is num 
	def  inner1():
		x = fun() # x= num()--> x=10
		return  x * x
	return  inner1
def   double(fun): # fun is inner1
	def  inner2():
		y = fun() # y= inner1(), y=100
		return  2 * y
	return   inner2
@double # num=double(square(num)) --- double(inner1)---inner2
@square # num=square(num) --- inner1
def  num():
	return  10
#end of the function
print(num()) # print(inner2())-->print(200)---> 200