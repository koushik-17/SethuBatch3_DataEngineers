...
class  triangle:
	def  get(self):
		...
		self.c = float(input('Enter third side: '))#How  to  read  three  sides  into  object  
	def  test(self):
		if (self.a + self.b >= self.c)and (self.b + self.c >= self.a) and (self.c + self.a >= self.c):
				pass
		else:
...
class Student:
    def get(self):
        self.r = int(input('Enter any roll no: '))
        self.s = input('Enter Student name : ')
        self.g = input('Enter gender (m/f): ')
        self.marks = []
        for i in range(3):
...
c.compute()
c.disp()
print(c.__str__())   # better way instead of c.__str__()