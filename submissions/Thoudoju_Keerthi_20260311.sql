...
select emp_name,dept_name from employees_demo e  inner join departments_demo d using(dept_id);
...
-- special queries
select emp_name,IFNULL(emp_name,'undetermined') from employees_demo;
select emp_name, replace(emp_name,'a','@') from employees_demo;
select trim(emp_name) from employees_demo;
select substring('*John*',2,length('*John*')-1);
select emp_id,
case
when emp_id > 105 then 'High '
when emp_id>100 then 'Med'
else 'Low'
end as id
from employees_demo;
select emp_name,substring(emp_name,3) from employees_demo;
select concat(emp_name," ", dept_name) from employees_demo join departments_demo using (dept_id);
select power(emp_id,2) as pow from employees_demo;
select emp_name from employees_demo e join departments_demo using (dept_id) where e.dept_id is null;
select emp_name from employees_demo where dept_id <>101;
select count(emp_name),dept_name from employees_demo e join departments_demo using (dept_id) where e.dept_id is not null group by dept_name;
select count(emp_name),dept_name from employees_demo e join departments_demo using (dept_id) where e.dept_id is not null group by dept_name having count(emp_name)> 1 ;
select count(emp_name),dept_name from employees_demo e join departments_demo using (dept_id) where e.dept_id is not null group by dept_name having count(emp_name)> 1 limit 2 offset 3;
select emp_name,dept_name from employees_demo e join departments_demo using (dept_id) where dept_name='IT'; 
select emp_name, dept_name,
case
when dept_name = 'IT' then dept_name
else null
end as dept from employees_demo e join departments_demo using (dept_id) ;
select emp_name, dept_name from employees_demo e join departments_demo using (dept_id) where dept_name= 'IT' and emp_id is not null and emp_name is not null;
