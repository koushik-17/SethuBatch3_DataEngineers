...
while  True:
	try:
		print(next(m)) # 10 <nl> 20  <nl> 15 <nl> 5 <nl> 18
		time . sleep(1)
...
2) Let  input  be   list  of  celsius  temperatures  such  as  [30 , 40 , 50 , 25]
    What  is  the  output ?  --->   1.8 * 30 + 32
								                  ...
								                  1.8 * 25 + 32
'''
celsius_list = eval(input("Enter list of celsius temperatures : "))
...
import time
radii = list(map(float, input("Enter list of radii : "))
m = map(lambda r: 3.14 * r * r, radii)
print("Areas of each radius in circles")
while True:
    try:
        print(next(m))
        time.sleep(1)
...
t1 = tuple(map(int, input("Enter first tuple :" ))
t2 = tuple(map(int, input("Enter second tuple : " ))
...
for  x   in    z2: 
        print(x) # for  loop (10, 4) (20, 5) (15, 6) (18, 7)
z3 = zip(list , cnt)
