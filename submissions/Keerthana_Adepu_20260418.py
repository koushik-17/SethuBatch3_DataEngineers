# What  are  k , l ,  x , y , z , m , n , p , q , s ?  (Home  work)
class   c1:
	x = 10  #  What  is  variable  'x'  ---> static variable of class c1
	def    m1(self):
		self . y = 20   #  What  is  variable  'y'  ---> instance variable
		z = 30   #  What  is  variable   'z'  ---> local variale of method m1
		c1 . m = 40   #  What  is  variable   'm'  ---> static variable of class c1
#end of the class
def    f1():
	a = c1()
	a . p = 50   #  What  is   variable  'p'  ---> instance variable of object a
	c1 . q = 60   #  What  is  variable   'q'  ---> static variable of class c1
	s = 70   #  What  is  variable   's'  ---> local variable of function f1
#end of the function
k = 80   #  What  is  variable 'k'  ---> global variable
c1 . l = 90   #  What  is  variable  'l'  ---> static variable of class c11
b = c1()
b . n = 100   #  What  is  variable  'n' ---> instance variable of object b


# Find  outputs (Home work)
class  Person:
	def  __init__(self):
		self . name  =  ''
	@property
	def   name(self):
		print('getter  method')
		return  self . _name
	@name . setter
	def   name(self , value):
		print('Setter  Method')
		self .  _name = value
	@name . deleter
	def  name(self):
		print('Deleter  method ')
		del  self .  _name
	def   __del__(self):
		print('Destructor')
# End  of  the  class
p = Person()
print(p . name)
p . name = 'Vamsi'
print(p . name)
del   p . name
print(p . name)
del   p


# Find  outputs (Home  work)
import   time
list = [25 , 10.8 , 'Hyd' , True]
e = enumerate(list , start = 5)
while   True:
	try:
		print(next(e))
		time . sleep(1)
	except  StopIteration:
		break
print(list[5]) # error , list has only 4 elements i.e. index ranges from 0-3
'''
(5 , 25)
(6 , 10.8)
(7 , 'Hyd')
(8 , True)
'''

#1
'''  (Home  work)
Can  string  be  enumerated ?
'''
import   time
a = input('Enter  any  string  :  ') #  Assume  that  input  is  'Hyd'
e = enumerate(a)
while   True:
	try:
		print(next(e))
		time . sleep(1)
	except  StopIteration:
		break
'''
(0 , 'H')
(1 , 'y')
(2 , 'd')
'''


#  Can  set  be  enumerated  ?  (Home  work)
import  time
a = {25 , 10.8 , 'Hyd' , True , 3+4j}
print(a) # {25 , 10.8 , 'Hyd' , True , 3 + 4j} in any order
b = enumerate(a)
while   True:
	try:
		print(next(b))
		time . sleep(1)
	except  StopIteration:
		break
'''
(0 , 25)
(1 , 10.8)
(2 , 'Hyd')
(3 , 'True')
(4 , 3 + 4j)
in any order
'''

# Can  dictionary  be  enumerated ?   (Home  work)
import   time
def  disp(e):
	while  True:
		try:
			print(next(e))
			time . sleep(1)
		except  StopIteration:
			break
	print()
dict = {'Empno'  :  25 , 'Emp Name'  :  'Rama Rao' , 'Sal' : 10000.0}
e1 = enumerate(dict . keys())
disp(e1)
e2 = enumerate(dict . values())
disp(e2)
e3 = enumerate(dict . items())
disp(e3)
e4 = enumerate(dict , start = 5)
disp(e4)
'''
(0 , Empno)
(1 , Emp Name)
(2 , Sal)

(0 , 25)
(1 , Rama Rao)
(2 , l0000.0)

(0 , (Empno , 25))
(1 , (Emp Name , Rama Rao))
(2 , (Sal , 10000.0))

(5 , Empno)
(6 , Emp Name)
(7 , Sal)
'''



# Find  outputs  (Home  work)
import time
a = ['Telangana' , 'Andhra  Pradesh' , 'Karnataka' , 'TamilNadu' , 'Maharastra']
b = ['Hyderabad' , 'Amaravathi' , 'Bangalore' , 'Chennai' , 'Mumbai']
e = enumerate(a)
for   index ,  element  in  e:
	print(F'{element:20}  ...  {b[index]}')
	time . sleep(1)

'''
Telangana (11 spaces) ... Hyderabad
Andhra Pradesh (6 spaces) ... Amaravathi
Karnataka (11 spaces) ... Bangalore
TamilNadu (11 spaces) ... Chennai
Maharastra (10 spaces) ... Mumbai
'''


