...
print(a . split(''))#Error because it is not valid
...
E) Write  a  program  to  evaluate  an  expression  which  contains  only  +  symbols
...
e = [15 , 36 , 48]
print(':' . join(e))#Error because there no ''
f = ['Sankar' , 'Dayal' , 'Sarma']
print('' . join(f))#'SankarDayalSarma'
g = range(5)
print('-' . join(g))#Error because it is not valid

G) Case  conversion  methods  
a = 'hyD  iS  grEen  cITy'
...
H) endswith()  method  demo  progrram
a = 'Hyd is green city'
...
I) Write  a  program  to  append  'ing'  to  input  string.
...
K) isdigit()  method  demo  program  (Home  work)
...
2) When  does  it  return  False ?  ---> When  at  least  one  character  of  the  string  is  a   non-digit  i.e. alphabet  (or) special  character			   (or)
When  there  are  no  digits  in  the  string

L) isspace()  method  demo  program  (Home  work)
print('\n  A\t' . isspace())#False
print('\n  \t' . isspace())#True
print('\n  7\t' . isspace())#False
print('\n' . isspace())#True
...
M) islower()  method  demo  program  (Home  work)
...
O) Write  a  program  to  determine  user  input  is  alphabet , digit , white space  or  special  character
...
7) What  is  the  output  if  input   is   <enter>   key ?  ---> W…
#Program
...
P) Write  a  program  to  reverse  a  string  without  slice
...
R) Write  a  program  to  reverse  each  word  of  the  sentence
...
S) Write  a  program  to  sort  string  such  that  alphabets  in  alphabetical  order  and  digits  in  ascending  order
