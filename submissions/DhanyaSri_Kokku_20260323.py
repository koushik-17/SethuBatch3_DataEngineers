...
print(add(10 , '20')) # Error because cannot concat int and str
...
f1(40 , 50)  # Error because three parameters are required for function f1()
f1(60)  # Error because three parameters are required for function f1()
f1() # Error because three parameters are required for function f1()
...
if  n == 0:
    ...
    res=''
    crore = n// 10000000 
    res += words(crore,"Crores")
    lakhs = (n // 100000) % 100
    res += words(lakhs,"Lakhs")
    thousand = (n // 1000) % 100
    res += words(thousand,"Thousand")
    hundred = (n // 100) % 10
...
f1(c = 3 + 4j , a = True , b = None) # a  :  True   \t  b  :  None  \t  c :  'Hyd'
f1(25 , c = 10.8 , b = 'Hyd') # a  :  25   \t  b  :  'Hyd'  \t  c :  10.8
f1(a = 100 , 200 , 300)  #   Error keyword argumnets cannot be used before positional arguments
f1(True , None , b = 'Hyd') # a  :  True   \t  b  :  'Hyd'  \t  c :  None
f1(10 , 20 , x = 30) # Error because of unknown argument x
f1(10 , 20) # Error because the f1() function requires three arguments not two
...
'''
Output:
Emp  Number : <2spaces>25  \t  Emp  Name : Rama Rao<7 spaces>  \t  Salary : 10000.0
Emp  Number : <2spaces>35  \t  Emp  Name : Sita<11 spaces>  \t  Salary : 20000.0
Emp  Number :  Rama Rao \t  Emp  Name : <8 spaces>30000.0  \t  Salary : 20
...
print(f1([6 , 7 , 8])) # Error because the f1() function requires three arguments and recieved only one
print(f1(*{1 : 2 , 3 : 4 , 5 : 6})) # print(16)
print(f1(**{'c' : 2 , 'b' :  4 , 'a' : 6})) # Error because the f1() function requires three arguments and recieved only one
print(f1({'c' : 2 , 'b' :  4 , 'a' : 6})) # Error because the f1() function requires three arguments and recieved only one
print({**{'c' : 2 , 'b' :  4 , 'a' : 6}}) # print({'c' : 2 , 'b' :  4 , 'a' : 6})
print(f1(**{'c' : 2 , 'a' : 4 , 'x' : 6})) # # Error because the f1() function requires three arguments and recieved only one

# Identify  Error (Home  work)
a = [10 , 20 , 15 , 5 , 12]
print(sorted(reverse = True , a)) # Here the keyword arguments cannot be used before positional arguments
print(sorted(a , rev = True)) # The sorted() function does not have the argument rev it has reverse argument
print(25 , 10.8 , 'Hyd' , separator = '\t') # The print() function does not have the argument separator it has sep argument
print(25 , 10.8 , 'Hyd' , endofline = '\t') # The print() function does not have endofline argument it has end argument
print(25 ,  sep = '\t' , 10.8 , end = '\t' , 'Hyd') # Here the positional arguments are defined after keyword arguments