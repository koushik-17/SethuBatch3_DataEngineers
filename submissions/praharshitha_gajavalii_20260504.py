...
ERROR becuase child class has no method m1() 
'''

# Identify  Error
class  c1(c1):
	     pass
# ERROR because c1 class is not fully defined yet,so it can't be inherited by itself
...
class   c1(c2):
	def  m1(self):
			super() . m1()
			print('Child  Method')
class  c2(c1): # ERROR because we can't create MRO for these classes
	def  m1(self):
			super() . m1()
...
class    parent:
	...
	def    m2(cls):
		...
		print(self . a)  # ERROR because no argument self 
	@staticmethod
	def   m3():
...
class  child(parent):
	...
	def   m2(cls):
		...
		cls . m2()  # ERROR because it causes infinite recursion
		self . m2() # ERROR because no argument self 
...
from  p1   import  mod1 . * # ERROR because we can't use . after import
