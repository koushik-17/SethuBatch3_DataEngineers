...
def powers_of_two():
    for i in range(8):  # 0 to 7 inclusive
        yield 2 ** i
...
1) 1st  string --->  'Sankar  dayal  sarma'
    2nd  string ---> 'san'
    What  are  the  outputs ?  --->  # san starts with Sankar dayal sarma

2) 1st  string  ---> 'Hyderabad'
    2nd  string  --->  'Sec'
    What  are  the  outputs ?  --->  # sec does not start with Hyderabad
'''
import re
...
def validate_mobile_number(number):
    ...
    if re.fullmatch(pattern, number):
        ...
        print(f"'{number}' is an INVALID mobile number.")
...
reg_no = input("Enter vehicle number: ")

pattern = r'^(TS|Ts|tS|ts)(0[1-9]|1[0-9]|2[0-9])[A-Za-z]{2}[0-9]{4}$'
