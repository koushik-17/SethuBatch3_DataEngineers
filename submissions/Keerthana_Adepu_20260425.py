...
'''
import re
mobile_number = input('Enter any Mobile Number:') 
m = re . fullmatch('(0|[+]91)?[6789][0-9]{9}', mobile_number)
if m:
...
What  is  the  regular  expression  for  the  above  rules ?  ---> (TS|Ts|tS|ts)([0][1-9]{1}|[1-2][0-9]{1})[a-zA-Z]{2}[0-9]{4}
'''
import re
vrn = input('Enter any Vehicle Registration Number:') 
m = re . fullmatch('(TS|Ts|tS|ts)([0][1-9]{1}|[1-2][0-9]{1})[a-zA-Z]{2}[0-9]{4}', vrn)
if m:
    print(f'{vrn} is Valid')
else:
    print(f'{vrn} is Invalid')
...
6) Regular Expression ---> (0?[1-9]|[12][0-9]|3[01])/(0?[1-9]|1[0-2])/[0-9]{4}
'''
import re
...
Regular Expression ---> [a-zA-Z ]+[,][a-zA-Z ]+[,][a-zA-Z ]+[-][0-9]{6}
'''
import re
...
Regular Expression ---> [456][0-9]{3}-?[0-9]{4}-?[0-9]{4}-?[0-9]{4}-?
'''
import re
ccn = input('Enter Credit Card Number:') 
m = re . fullmatch('[456][0-9]{3}-?[0-9]{4}-?[0-9]{4}-?[0-9]{4}-?', ccn)
if m:
    print(f'{ccn} is Valid')
else:
    print(f'{ccn} is Invalid')
