...
2.) what are pseudo columns which are using for sequences.
 ...
 nextval: give next number sequence 
 currval: give current number


3.)can we create a view without table.

Normally, we can't create view without a base table
  but in oracle, we can create view using force without any base table
...
we will get 8 records from view
