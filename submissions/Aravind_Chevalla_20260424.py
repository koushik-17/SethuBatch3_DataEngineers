...
for  x  in   itr: 
	print(x)
# __iter__() returning self which is iterator internally it should execute __next__(), 
# but c4 class has no __next__() so return object is not iterator and its invalid.
...
for  x  in   itr:
	print(x)
# __iter__() is not returning anything so by default it returns None 
# But for loop asks __iter__() to return iterators only.So its an error.
...
a  =  c6()
print(dir(c6))
for  x  in  a:   # searches for __iter__() and __next__() but no such method
        print(x)
while  True:
	print(next(a))  # Searches for __next__() but no such method so error
a . next()
...
iS  is  between  indexes  52  and  53
Found  4  times
'''
