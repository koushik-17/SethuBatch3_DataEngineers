# Find outputs  (Home  work)
print( 'green'   in   'Hyd  is  green  city') # True
print('day'   in   'Sankar  dayal  sarma') # True
print('Green'   in   'Hyd  is  green  city')# False
print('d  is'   in   'Hyd  is  green  city')# True
print('dis'   in   'Hyd  is  green  city')# False
print('iniv'   in   'Srinivas')# True
print('iniv'   not   in   'Srinivas')# False

# ==============================================
'''  (Home  work)
Slice  demo  program
0      1       2        3      4       5       6       7
R      a      m        a               R       a       o
-8    -7     -6      -5     -4      -3     -2      -1
'''
a = 'Rama Rao'
print(a [ : 7 : 2]) #[0 : 7 : 2]  --->  string  from  indexes  0  to  6  in  steps  of  2  i.e. Rm<space>a
print(a [ : 7]) #[0 : 7]  --->  string  from  indexes  0  to  6 in steps of 1  i.e. Rama Ra
print(a [2 : 4]) #[2 : 4]  --->  string  from  indexes  2  to  3 in steps of 1 i.e. ma
print(a [2 : ]) #[2 : 8]  --->  string  from  indexes  2  to  7 in steps of 1 i.e. ma Rao
print(a [ : 4 ]) #[0 : 4]  --->  string  from  indexes  0  to  3 in steps of 1 i.e. Rama
print(a [ : : 2])  #  a[0 : 8 : 2]   --->  string  from  indexes  0  to  7  in  steps  of  2  i.e. Rm<space>a
print(a [-6 : -1]) # a[-6 : -1]  --->  string  from  indexes  -6  to  -2 in steps of 1 i.e. ma Ra
print(a [-6 : ]) # a[-6 : 8]  --->  string  from  indexes  -6  to  7 in steps of 1 i.e. ma Rao
print(a [: -4 : -1])# a[0 : -4 : -1]  --->  string  from  indexes  0  to  -5 in steps of -1 i.e. aR
print(a [-3 : -1])# a[-3 : -1]  --->  string  from  indexes  -3  to  -2 in steps of 1 i.e. Ra
print(a [-3 : ])# a[-3 : 8]  --->  string  from  indexes  -3  to  7 in steps of 1 i.e. Rao
print(a [ : : ])# a[0 : 8 : 1]  --->  string  from  indexes  0  to  7 in steps of 1 i.e. Rama Rao
print(a [ : ])# a[0 : 8 : 1]  --->  string  from  indexes  0  to  7 in steps of 1 i.e. Rama Rao
print(a [ : : -1])# a[0 : 8 : -1]  --->  string  from  indexes  0  to  7 in steps of -1 i.e. oaR amaR
print(a [ : : -2])  #  a[-1 : -9 : -2]  --->  string  from  indexes  -1  to  -8  in  steps  of  -2  i.e.  oRaa
print(a [ -2 : : -2]) # a[-2 : -9 : -2]  --->  string  from  indexes  -2  to  -8  in  steps  of  -2  i.e. aR
print(a [2 : 8])# a[2 : 8]  --->  string  from  indexes  2  to  7 in steps of 1 i.e. ma Rao
print(a [2 : 8 : -1])# a[2 : 8 : -1]  --->  string  from  indexes  2  to  7 in steps of -1 i.e. aR
print(a [ : -6 : -1])# a[0 : -6 : -1]  --->  string  from  indexes  0  to  -7 in steps of -1 i.e. oar a
print(a [2 : -3])# a[2 : -3]  --->  string  from  indexes  2  to  -4 in steps of 1 i.e. ma 
print(a [1 : 6 : 2])# a[1 : 6 : 2]  --->  string  from  indexes  1  to  5 in steps of 2 i.e. a aR
print(a [ : -5 : -5])# a[0 : -5 : -5]  --->  string  from  indexes  0  to  -6 in steps of -5 i.e. o
print(a [2 : -5]) #[2:-5]  --->  string  from  indexes  2  to  -6 in steps of 1 i.e. m
print(a [2 : -5 : 2])# a[2 : -5 : 2]  --->  string  from  indexes  2  to  -6 in steps of 2 i.e. m
print(a [ : 0 : -1]) # a[0 : 0 : -1]  --->  string  from  indexes  0  to  -1 in steps of -1 i.e. aRao amaR
print(a [-5 : 0 : -2])# a[-5 : 0 : -2]  --->  string  from  indexes  -5  to  -1 in steps of -2 i.e.aa

#============================================================
'''
Write  a  program  to  concatenate  two  strings  separated  by  space  but  swap  first  two
characters  of  the  two  strings.
Assume  that  each  string  has  a   minimum  of  two  characters

Let  inputs  be  Java  and  Python
What  are  the  outputs ?  ---> Pyva<space>Jathon

Hint:  Use  slice
'''
s1 = input('Enter  first  string : ')
s2 = input('Enter  second  string : ')
s3 = s2 [ : 2] + s1 [2 : ] + ' ' + s1 [ : 2] + s2 [2 : ]
print('Output : ', s3)

# ===========================================================
'''
Write  a  program  to  print  first  two  and  the  last  two  characters  of  the  string
Print  an  empty  string  if  string  has  less  than  four  characters

1) Let  input  be  PYTHON
    What  is  the  output ?  ---> PYON

2) Let  input  be  Hyd
    What  is  the  output ?  ---> Nothing
'''
s = input('Enter  a  string : ')
if len(s) < 4 :   
    print('Output : ', '')
else :    
    s1 = s [ : 2] + s [-2 : ]
    print('Output : ', s1)
'''
Write  a  program  to  print  characters  of  the  string  in  forward  and  reverse  directions  without  slice

       	     				  0      1     2      3     4
Let  input  be		  V     A     M     S     I
			        		 -5    -4    -3    -2    -1

What  are  the  outputs ?  --->  Character  at  index  0  :  V
								                 Character  at  index  1  :  A
								                 Character  at  index  2  :  M
							                     Character  at  index  3  :  S
								                 Character  at  index  4  :  I

								                 Character  at  index  -1  :  I
								                 Character  at  index  -2  :  S
								                 Character  at  index  -3  :  M
								                 Character  at  index  -4  :  A
								                 Character  at  index  -5  :  V

Hint:  Use  two  for  loops
'''
a= input('Enter  a  string : ')
print('Characters  in  forward  direction : ')
for i in range(len(a)) :
    print('Character  at  index  ', i, ' : ', a[i])     
print('Characters  in  reverse  direction : ')
for i in range(-1, -len(a)-1, -1) :
    print('Character  at  index  ', i, ' : ', a[i])

'''
Write  a  program  to  print  characters  at  even  and  odd  indexes  without  slice

							 0      1      2      3     4     5     6      7
Let  input  be        R      a     m      a             R     a      o

odd =  '' + 'a' + 'a' + 'R' + 'o' = 'aaRo'
even =   '' + 'R' + 'm' + ' ' + 'a' = 'Rm a'

1) What  action  to  be  made  when  index  is  even ?  ---> Concatenate  the  character  to  even  object

2) What  action  to  be  made  when  index  is  odd ?  --->  Concatenate  the  characeter  to  odd  object

3) Hint: Use  single  for  loop
'''
a= input('Enter  a  string : ')
odd = ''
even = ''
for i in range(len(a)):
    if i % 2 == 0:
        even += a[i]
    else:
        odd += a[i]
print('Characters  at  even  indexes : ', even)
print('Characters  at  odd  indexes : ', odd)

'''
Let  input  be    A   4   B   3   C   2   $   5
                         0   1    2   3   4   5   6   7

What  is  the  output ?  --->  AAAABBBCC$$$$$

1) What  is  the  result  of  'A' * 4  ?  ---> 'AAAA'

2)  i        a[i]       a[i + 1]          out
   -------------------------------------------------------
                                               ''
    0         'A'          '4'			   '' + 'A' * 4 = 'AAAA'
	2         'B'          '3'             'AAAA' +  'B' * 3  = 'AAAABBB' 
	4         'C'          '2'             'AAAABBB' +  'C' * 2  = 'AAAABBBCC' 
	6         '$'          '5'             'AAAABBBCC' +  '$' * 5  = 'AAAABBBCC$$$$$' 
   -------------------------------------------------------
What  is  the  difference  between  a[i]   and  a[i + 1] ?  --->  a[i]  is  ith  char  of  string  and  a[i + 1]  is  (i + 1)th  char  of  string
'''
a = input('Enter  a  string : ')# a4b3c2$5
out = ''
for i in range(0, len(a),2):
    out += a[i] * int(a[i + 1])   
print('Output : ', out)

'''
Write  a  program  to  merge  two  strings  to  form  a  new  string

1) Let  inputs  be    HYD   and   VAMSI
   What  is  the  output  ?  ---> HVYADMSI

             0     1    2
a  --->   H     Y    D

              0    1     2     3    4
b  --->    V    A    M    S    I


i       a[i]        b[i]          c
--------------------------------------------------------
                                    ''
0       'H'        'V'         '' + 'H' + 'V = 'HV'									
1       'Y'        'A'         'HV' + 'Y' + 'A = 'HVYA'									
2       'D'        'M'        'HVYA' + 'D' + 'M = 'HVYADM'									
--------------------------------------------------------

Concatenate  remainging  characters   of  the  other  string  to  object  'c'
What  is  the  final  result ?  --->  'HVYADMSI'

Hint:  Use  single  while  loop  and  slice
'''
a= input('Enter  first  string : ')# HYD
b= input('Enter  second  string : ')# VAMSI
c = ''
i = 0
while i < len(a) and i < len(b):
    c += a[i] + b[i]
    i += 1
c += a[i:] + b[i:]
print('Merged string : ', c)
# ==================================================================
'''
Modify  following  program  with  walrus  operator

Hint:  Combine  lines  7 , 8  and  10  to  a  single  line  with  walrus  operator
'''
a = 'Hyd is green city. Hyd is hitec city. Hyd is his cityi'
index = -1
while (index := a.find('is', index + 1)) != -1:
    print(index)

print('End')

# =======================================================================
'''
index()  method
-------------------
It  is  same  as  find()  method  except  that  it  raises   error (but  not  -1) when  string  is  not  found

Syntax :  Same  as   find()  method
'''

a = 'Hyd is green city. Hyd is hitec city. Hyd is his cityi'
b = 0

try:
    while True:
        b = a.index('is', b)
        print(b)
        b += 1
except ValueError:
   pass
print('End')

# ======================================================================
'''(Home  work)
rfind()  method  demo  program

Modify  following  program  with  rfind()  method
'''
a = 'Hyd is green city. Hyd is hitec city. Hyd is his cityi'
index = a . find('is')
while  index != -1:
	print(index)
	index=a.rfind('is', 0, index)
print('End')

'''
rfind()  method
-------------------
1) What  does  str1 . rfind(str2 , x , y)  do ?  --->  Returns  index  of  str2  in  str1   between  indexes  y - 1  downto  x
																			  i.e. right  to  left

2) What  does  str1 . rfind(str2)  do ?  --->  Returns  index  of  str2  in  str1   between  indexes  len(str1) - 1  downto  0
																     i.e. right  to  left

3) What  does  str1 . rfind(str2 , x)  do ?  --->  Returns  index  of  str2  in  str1   between  indexes  x  to  len(str1) - 1
														                  i.e.  left  to  right

4) How  many  arguments  can  rfind()  method  take ?  --->  1 (or) 3  but  not  2

5) What  is  the  issue  with  two  arguments ?  ---> Method  searches  from  left  to  right  even  though  it  is  rfind()  method

6) What  does  rfind()  method  return  (+ve  (or)  -ve  index) ?  ---> +ve  index  even  though  search  is  from  right  to  left

7) What  does  rfind()  method  do  if  str2  is  not  in  str1 ?  --->  Returns  -1
'''

# ============================================================
#  count()  method  demo  program (Home  work)
a = 'Hyd is green city. Hyd is hitec city. Hyd is his city'
print(a . count('is')) #4
print(a . count('is' , 19 , 48)) # 3
print(a . count('was'))# 0


'''
count()
---------
1) What  does  str1 . count(str2)  do ? --->  Returns  number  of  times  str2  is  in  str1

2) What  does  str1 . count(str2 , x , y)  do ? --->
																	Returns  number  of  times  str2  is  in  str1  between  indexes  x  and  y - 1
'''
#  Find  outputs  (Home  work)
a = 'Hyd is\tgreen\ncity.Hyd is\thitec\ncity.Hyd is\this\ncity'
print(a . count(' '))
print(a . count('\t'))
print(a . count('\n'))