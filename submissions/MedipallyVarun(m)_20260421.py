...
f2 = filter(lambda x: x % 2 == 0, a) #How  to  iterate  thru  filter  object  with  next()  function
while True:
    try:
        print(next(f2))
        time.sleep(1)
...
f3 = filter(lambda x: x % 2 == 0, a)#How  to  iterate  thru  filter  object  with  for  loop
for x in f3:
	print(x)
	time.sleep(1)
...
'''
def distinct_vowels(a):
    vowels = 'AEIOU'

    filtered_vowels = filter(lambda char : char in vowels, a)
    set_vowels = set(filtered_vowels)
