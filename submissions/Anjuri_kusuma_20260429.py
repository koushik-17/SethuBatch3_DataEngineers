...
try:
    con=mc.connect(host="localhost",database='emp',user='root',password="Teji@1836")
    cur=con.cursor()
    cur.execute("select * from employee")
    n=int(input("Enter no. of rows to fetch data  : "))
    for col in cur.description:
        print(F'{col[0]: ^10}',end='\t')
    print()
    while True:
        tpl=cur.fetchmany(n)
        if not tpl:
...
try:
    ...
    while True:
        ...
        cur.execute("insert into employee values(%s,%s,%s)",(empno,ename,sal))
        con.commit()
        print("Row inserted, count: ",cur.rowcount)
    ...
    print("Invalid query or table")
except mc.errors.InterfaceError:
    print("Start MySQL Server3")
---------------------------------------------------------------------------------
'''
...
try:
    con=mc.connect(host='localhost',database='emp',user='root',password='Teji@1836')
    cur=con.cursor()
    cond=input("Enter the condition (e.g.,salary>50000): ")
    query=f"delete from employee where {cond}"
    cur.execute(query)
    con.commit()
...
try:
    con=mc.connect(host='localhost',database='emp',user='root',password='Teji@1836')
    cur=con.cursor()
    expr=input("Enter updation  :")
    cond=input("Enter the condition : ")
    query=f"update employee set {expr} where {cond}"
    cur.execute(query)
    con.commit()
