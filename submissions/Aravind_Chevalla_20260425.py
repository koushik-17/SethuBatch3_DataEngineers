...
if re.fullmatch('(0|[+]91)?[6789][0-9]{9}', mob):
...
'''
import re
vnum = input('Enter a vehicle number : ')
if re.fullmatch('(TS|ts|Ts|tS)(0[1-9]|1[0-9]|2[0-9])[A-Za-z]{2}[0-9]{4}', vnum):
...
'''
import re
date = input('Enter a date in dd/mm/yyyy form : ')
if re.fullmatch('(0?[1-9]|[1][0-9]|2[0-9]|3[01])/(0?[1-9]|1[0-2])/[0-9]{4}', date):
    print('Valid date')
else:
    print('Invalid date')
...
Sample output:
Enter address : Khairtabad,Hyd,Telangana-500004
Valid address
'''
import re
address = input('Enter address : ')
if re.fullmatch('[A-Za-z ]+,[A-Za-z ]+,[A-Za-z ]+-[0-9]{6}', address):
    print('Valid address')
else:
...
if re.fullmatch('([456][0-9]{15})|([456][0-9]{3}(-[0-9]{4}){3})', num):
