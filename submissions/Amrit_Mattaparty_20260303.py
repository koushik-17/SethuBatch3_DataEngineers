...
try:
    sum = ctr = 0
    # The walrus operator (:=) assigns the input to 'x' AND checks if it exists
...
3)  py prog3.py
    What is the output? ---> Please send an integer input
...
2)  py prog4.py
    What is the output? ---> Please send number inputs
...
3)  py prog5.py  
    What is the output? ---> Please send inputs

3)  py prog5.py 'Hyd' 'Sec' 'Cyb'
    What are the outputs? ---> ['Cyb' , 'Hyd' , 'Sec'] ['Sec' , 'Hyd' , 'Cyb']
'''
import sys
