...
"""
try:
    a = input("Enter a Sting with integers : ").split("+")
    sum = 0
    for b in a:
        sum= int(b) + sum
    print(f"Sum: {sum}")
...
print('a  :  {2}  \t  b  :  {2}  \t  c  :  {2}  ' . format(a , b , c)) #{[a: 'Hyd'], [b: 'Hyd'], [c: 2'Hyd'5]}
...
'''
"""
j = input("Enter any string : ").split()
k =[]
for l in range(1,len(j)+1):
     k .append(j[-l])
m = " ".join(k)
...
'''
 "AEIOU"
if(Write  a  program  to  sort  string  in  alphabetical  order
...
'''
"""
g = input("Enter any string : ")
h = sorted(g)
print(f"Sorted String : {"".join(h)}")
"""
...
'''
"""
c = input("Enter a string : ").upper()
d = "AEIOU"
e = ""

for a in c:
    if a in d and a not in e:
        e += a
