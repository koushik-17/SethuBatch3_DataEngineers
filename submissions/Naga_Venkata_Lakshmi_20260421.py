def   rotate(x):  #  'a'  is  list
	global a
	a = x[-1:] + x[:-1]
	return a
#  End  of  the  function	
a=eval(input("enter list :"))
k=eval(input("Enter k value :)")) 
while k:
	print(rotate(a))
	k-=1
...
disp(f1)# 10 -25 (25,) Hyd 10.8 [10,20] True
f2 = filter(None  , list)
print('Filter  f2')
disp(f2)
...
while   True:
	try:
		print(next(f))# Rama Rao , Rajesh , Kkran , Manohar , Vamsi
		time . sleep(1)
	except:
		break
...
import time
n=int(input("Enter upto N value : "))
list=[x for x in range(1,n+1)]
f1=filter(lambda x:x%2==1, list)
for y in f1:
    print(y)
    time.sleep(0.5)
...
s=set()
f1=filter(lambda x: x in 'AEIOU' or x in 'aeiou', set(str))
for y in f1:
    print(y)
