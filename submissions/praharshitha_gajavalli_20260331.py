...
print(sorted(a))# Error because we can't compare dictionary
...
def  f1():
	global  a
	if  a:
		print(a) # 3<nextline>2<nextline>1<nextline>Bye
		a = a - 1
		f1()
		print('Hello')
		print('Hi')
...
outer()
print('Hi')
inner() # Error 'inner' is defined inside another function
other()
print('Bye')
'''
Begin
...
outer()
print(x) # Error because global x is not defined
'''
10
15
20
...
def  outer():
	def  inner():
		nonlocal  x # Error because outer function doesn't have local variable
		x = 20
		print(x)
	# End  of  inner  function
	inner()
...
#  Identify  Error
def   f1():
        nonlocal   x # Error because nonlocal keyword can be used in inner loop only
...
def   outer():
	...
	def  inner():
		global   x
		nonlocal  x # Error because x can't be global and nonlocal at the same time
