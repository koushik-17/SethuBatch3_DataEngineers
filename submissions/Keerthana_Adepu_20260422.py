...
2) Let  input  be   list  of  celsius  temperatures  such  as  [30 , 40 , 50 , 25]
    What  is  the  output ?  --->   1.8 * 30 + 32
								                  ...
								                  1.8 * 25 + 32
'''
t = eval(input('Enter the list with celsius temperatures:'))
f = map(lambda x : 1.8 * x + 32 , t)
for x in f:
    print(x)
...
'''
import math
r = eval(input('Enter the list with radius of circles:'))
a = map(lambda x : f'{math . pi * x * x:.2f}' , r)
for x in a:
    print(x)
...
max = reduce(lambda x , y: max(x , y), m)
print(max)
...
ans = reduce( lambda  x , y  : x + y , map(lambda  y :  y ** 2 , filter(lambda  x  :  x  >= 15 , a))) # filter: 20 , 15 , 18 , 25 # map: 400 , 225 , 324 , 625 
print(ans) # 400 + 225 + 324 + 625
...
Memory error because Python object cannot hold infinite elements
'''
...
disp(d)
'''
10 <tab> 11 <tab> 12 <tab> 13 <tab>
10 <tab> 15 <tab> 20 <tab> 25 <tab>
10 <tab> 7.5 <tab> 5 <tab> 2.5 <tab>
0 <tab> 1 <tab> 2 <tab> 3 <tab>
'''
