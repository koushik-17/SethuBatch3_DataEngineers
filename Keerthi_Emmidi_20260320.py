...
dict()  function
------------------
1) What  is  the  argument  of  dict()  function ?  --->Nested  sequence  such  as  list  of  tuples , list  of  lists , tuple  of  tuples , tuple  of  lists,set  of  tuples  and  so  on
...
data = input("Enter dictionary items (key:value pairs separated by commas): ")
data_dict = {}
for item in data.split(","):
    key, value = item.split(":")
    data_dict[int(key)] = value
sorted_dict = {key: data_dict[key] for key in sorted(data_dict)}
...
for ch in chars:
    if ch.isalpha():  
        a[ch] = a.get(ch, 0) + 1
print("Frequency dictionary:", dict(sorted(a.items())))
...
print(b) # {15: 'Sita', 17: 'Kiran'} → only odd keys
c = {k : a[k]   for   k   in    a    if   a[k] . startswith('R')}
print(c) # {10: 'Rama', 18: 'Rajesh', 12: 'Rama Rao'} → only values starting with 'R'